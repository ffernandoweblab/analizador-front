...
export default function Productividad() {
  ...
  // ── Cargar predicción cuando se selecciona mañana ──
  useEffect(() => {
    if (!esManana) {
    ...
    Promise.all([
      ...
      .then(([pred, hoy]) => {
        ...
        // Construir mapa userId -> nombre desde los datos de hoy
        if (hoy?.users) {
          const map = {};
  ...
  const cargar = useCallback(async () => {
    if (esManana) return; // mañana usa su propio fetch
    setLoading(true);
    setErr("");
    try {
                ...
                <Stack spacing={{ xs: 2, sm: 3 }}>
                  <Box sx={{ overflowX: { xs: "auto", sm: "visible" }, WebkitOverflowScrolling: "touch", pb: { xs: 0.5, sm: 0 } }}>
                    <ToggleButtonGroup
                      ...
                      sx={{ flexWrap: { xs: "nowrap", sm: "wrap" }, gap: 1, width: { xs: "max-content", sm: "100%" }, "& .MuiToggleButton-root": { borderRadius: 2, border: "1px solid", borderColor: "divider", textTransform: "none", fontWeight: 600, px: { xs: 2, sm: 2.5 }, py: 1, whiteSpace: "nowrap", "&.Mui-selected": { background: "linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%)", color: "white", borderColor: "transparent", "&:hover": { background: "linear-gradient(135deg, #2563eb 0%, #7c3aed 100%)" } } } }}
                    >
            ...
            {esManana ? (
              loadingPrediccion ? (
                ...
                <>
                  ...
                  {/* Tarjetas individuales predicción */}
